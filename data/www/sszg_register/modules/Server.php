...
class Server extends App
{
    ...
    */
    ...
    {
        $platform = Api::getVar('platform', 'POST');
        $channel_id = Api::getVar('chanleId', 'POST');
        $account = Api::getVar('account', 'POST'); //角色账号
        $last_srv_id = Api::getVar('srvid', 'POST');
        $startIdx = Api::getInt('start', 'POST');
        $num = Api::getInt('num', 'POST');
        $sign = Api::getVar('sign', 'POST');
        $timeZone = Api::getInt('timeZone', 'POST');
        $time = Api::getInt('time', 'POST');
        //关键参数验证
        if(empty($account) || empty($sign) || empty($timeZone) || empty($time)) {
            Api::log('缺少请求参数 account, sign , timeZone , time', 'sszg_reg', 'request');
            Api::out(1, '请求参数错误');
        };
        if ($channel_id == '') Api::out(2, '渠道号不能为空');
        //sign值校验
        if ($sign !== md5($channel_id . $time . $timeZone . $account . '232fgD0f3afBa1Tst88bH05727DFc' )) {
            Api::log($account.'验证sign错误', 'sszg_reg', 'request');
             Api::out(3, '验证错误');
        }
        //设置时间请求有效期
        $diffTime = strtotime(gmdate('Y-m-d H:i:s', time() + ($timeZone * 3600)));
        if($diffTime - $time > 60) {
            Api::log($account.'时间超过60秒', 'sszg_reg', 'request');
            Api::out(4, '时间验证错误');
        }
    ...
    public function all_servers()
    {
        ...
        from game_servers where {$where} order by open_time desc");
        $other_servers = array(
        //    array('platform'=>'release', 'zone_id'=>1, 'name'=>'稳定服', 'host'=>'s1.release.huanxiang.shiyuegame.com','port'=>9611,'open_time'=>0)
        ); 
