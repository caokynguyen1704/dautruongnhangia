...
class GM_GameApi
{
     ...
     * @return array
     */
    public function maill($roles, $Title, $Content,$Items, $adminName)
    {
        $type = is_int($roles[0][0]) ? 'i' : 'b';
        $type = implode(', ', array_fill(0, count($roles), "{{$type}, b, i}"));
		$itemArg = array();
		$itemArg[] = "{i,i,i}";
		$itemStr = implode(',', $itemArg);			
        $ret = erl('adm', 'a0f6a30ffed2e8b95634486728632534', "[[{$type}],b,b,[{$itemStr}],b]" , [
		$roles, 
		$Title, 
		$Content,
		$Items, 
		...
		{
            return API::returnMsg('MAIL_HANDLE_FAILURE', $ret['message']);
        }		 
