...
cfg_init(){
    # 节点根目录
    cfg['root']="/data/zone/sszg_local_1"
    # 代码存放路径，每一个版本一个目录保存到此路径下
    cfg['code_path']="/data/sszg_code/"
    # 平台标识
    cfg['platform']="local"
    # 区号
    cfg['zone_id']="1"
    ...
    cfg['type']="zone"
    # magic cookie
    cfg['cookie']="11cyx58dsdmxyTjl08Bxerycw"
    # 主域名(这里不要带http://)
    cfg['host']="20.230.41.48"
    # 合服域名(这里不要带http://)
    cfg['merge_host']=""
    # 监听端口
    cfg['port']="8001"
    ...
    # 服务器密钥
    cfg['srv_key']="87591ec04aebdda12e1d960db30fde1as1"
    ...
    # 节点名称
    cfg['nodename']="sszg_local_1@127.0.0.1"
    # 中央服节点名称
    cfg['center_node']="sszg_center_0@127.0.0.1"
    # 所在物理机器编号
    cfg['machine']="11007"
    # 该类型节点的扩展信息
    cfg['ext']="{{ext}}"
    ...
    # 游戏代号
    cfg['game_name']="shiningevolution"
    # 游戏区名称
    cfg['zone_name']="shiningevolution"
    # 语言版本
    cfg['lang']="zh-CN"
    # 时区
    ...
    cfg['combine']=''
    # 开服时间
    cfg['open_time']="1643338800"
    # 合服时间
    cfg['merge_time']="1643338800"
    ...
    cfg['flume_host_group']="1"
}

# 启动节点
fun_start(){
    if [ ! -e dets ]; then
        echo ">> [ Sai lầm ] Trước mắt mục lục hạ chưa lắp đặt tiết điểm, mời trước chấp hành lắp đặt thao tác"
        exit 1
    fi
...
EOF
        ...
        echo ">> Tiết điểm ${cfg['nodename']} Ngay tại khởi động bên trong, nếu như muốn quan sát khởi động quá trình mời sử dụng trở xuống mệnh lệnh tiến hành khởi động:"
...
## 命令行帮助
help(){
    echo "start              Khởi động tiết điểm"
    echo "stop              Quan bế tiết điểm ( Đồng thời sẽ tự động thanh lý php session)"
    echo "shell              Tiến vào đài điều khiển, cũng có thể sử dụng screen Mệnh lệnh trực tiếp tiến vào"
    echo "remsh              Lấy remsh Phương thức tiến vào đài điều khiển"
    echo "install            Lắp đặt tiết điểm"
    echo "uninstall          Xóa bỏ tiết điểm"
    echo "clean              Thanh ngăn thao tác"
    echo "clean_sess        Thanh lý php session, quan bế Server sau cần thanh lý, không phải người chơi đổi mới sau khả năng tiến vào Server"
    echo "update_cfg        Đổi mới tất cả phối trí văn kiện"
    echo "hotswap            Nóng đổi mới tiết điểm"
    echo "exec              Chấp hành chỉ định mệnh lệnh: Mod:Fun:Args"
    echo "pack              Thu thập cũng đóng gói nhật Chí Văn kiện"
    echo "log                Xem xét đài điều khiển nhật ký"
    echo "check              Phán đoán tiếp điểm phải chăng còn đang vận hành bên trong"
    echo "ls                Liệt xuất xứ có erl Tiết điểm"
    echo "run_db            Kiểm tra tiết điểm phải chăng đang vận hành trạng thái nhưng kho số liệu đã bị xóa bỏ ( Giữ gìn công cụ phụ trợ )"
}
...
case $1 in
    ...
    run_db) fun_run_db;;
    *)
        echo "Không biết chỉ lệnh, mời sử dụng trở xuống hữu hiệu chỉ lệnh"
        echo "----------------------------------------------------------"
        help
        exit 1
        ;;
