...
if BUILD_VERSION == 3 then
  APP_ROUTE="app"
  PLATFORM_PNG=APP_ROUTE
  IS_SY_GAME=true
  SHOWVERSIONMSG = "新广出审[2018]1192号\nISBN：978-7-498-04567-6\n著作权人：广州纷至网络科技有限公司\n出版服务单位：上海同济大学电子音像出版社有限公司"
end
<?php 
function getIP()
{
...
$is_verifyios = true;
// if(($build_ver >= 3 && ! $is_verifyios) || $verify_ip){
//     echo("FINAL_SERVERS = FINAL_SERVERS or {platform = 'release2', zone_id = 2, isnew = 0, name = MAKELIFEBETTERSERVERNAME, host = 's2-release2-sszg.shiyuegame.com', port = '11003', begin_time = 0}");
// }
if(($build_ver >= 5 && $is_verifyios) || $verify_ip){
ob_clean();
?>
